    ...
    async def _execute_dsh_task(
        ...
        if cfg.persona.mode_name == "custom" and cfg.persona.custom_system_prompt.strip():
            ...
            final_prompt = (
                f"【用户具体任务】\n{task}\n\n"
                "【严格执行与回答规范】：\n"
                "1. 任务开始前：优先检查并查阅 /main/skill 目录下及项目相关的 skill / memory.md 文件，看是否有之前积累的相关历史经验与操作规则；\n"
                "2. 任务执行中：使用 bash/read/glob 等工具实际执行并检查真实目标（如涉及 git 状态、日志分析、文件排查），获取第一手事实；\n"
                "3. 文件与模块规范：为了避免单个代码/文档文件过大，严禁堆砌单文件，必须在合理模块目录下做好分文件放置与清晰的模块目录分类，并在主索引/README 中维护好对应的检索索引；\n"
                "4. 最终交付：必须直接、清晰地回答用户所提问的具体任务目标，直接给出分析结论与数据，严禁答非所问，严禁生成无关的架构图或流程图；\n"
                "5. 任务完成后：总结本次任务中具有复用价值的通用经验与关键变更，并在必要时同步更新相关 skill 或 memory.md，保持经验闭环。"
            )
